...
def list_files(directory: Path, allowed_exts: tuple[str, ...]) -> Iterable[Path]:
    files = sorted((p for p in directory.iterdir() if p.is_file() and p.suffix.lower() in allowed_exts), key=lambda p: p.name)
...
def list_dirs(directory: Path) -> Iterable[Path]:
    ...
    logger.debug("Found %d subdirectories in %s", len(dirs), directory)
    return dirs
