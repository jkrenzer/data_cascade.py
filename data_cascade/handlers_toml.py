...
class TomlHandler(FileHandler):
    ...
    def can_handle(self, path: Path) -> bool:
        return _TOML_LOAD_BACKEND is not None and path.suffix.lower() in self.supported_exts()

    def load(self, path: Path) -> Any:
        if _TOML_LOAD_BACKEND is None:
            raise RuntimeError("No TOML backend available. Install tomli or use Python 3.11+.")
...
if _TOML_LOAD_BACKEND is not None:
    ...
    logger.debug("Using TOML backends: load=%s save=%s", _TOML_LOAD_BACKEND, _TOML_SAVE_BACKEND or "NONE")
else:
    logger.warning("TOML files will not be loaded because no TOML library is available.")
